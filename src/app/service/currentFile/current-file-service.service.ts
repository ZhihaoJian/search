...
export class CurrentFileServiceService {
   ...
   */
  public getFirstSelectionGrid(url: string, tableName: string, catalogueId: string) {


    const tableNameArray = {};

    return new Promise((resolve, reject) => {
      this.getTableHeader('t_guidangwenjianmulu')
        .then(res => {
          if (res) {
        ...
        .then(res => {
          this.getDataFromDb('t_guidangwenjianmulu', url, catalogueId, tableNameArray, (res as any).en)
            .then(response => {
  ...
  // 从数据库获取数据
  getDataFromDb(tableName: string, url: string, catalogueId: string, tableNameArray: object, tableNameEns: Array<string>) {
